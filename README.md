...
(default `~/.framework_reader/`), **not in `build/`** - `make clean` deletes all
...
The home page is a search workbench: a sentence or a control number; if the
literal search misses, AI finds nearby phrasings. When AI drafts are waiting for
sign-off, a card shows "AI drafts awaiting confirmation" leading to the
**review queue** - one at a time, arrow keys to page, each still signed
individually (there is no bulk confirm). Below that: frequently searched
controls, and three interpreted controls a day to learn from. Built-in and
imported frameworks live under "Frameworks" in the top bar, with "has
...
Imported frameworks go into `~/.framework_reader/user.sqlite`, **never into the
